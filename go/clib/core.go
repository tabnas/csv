...
//
// libtabnascsv is the csv format parser as a C shared library, one of
...
import (
	...
	plug "github.com/tabnas/csv/go"
	host "github.com/tabnas/jsonic/go"
)

const (
	templateVersion = "v1"
	libName         = "libtabnascsv"
	formatName      = "csv"
	valueOut        = true
)
...
// newParser builds one engine with the csv grammar installed
...
func newParser() (parseFn, error) {
	j := host.Make(); if err := j.UseDefaults(plug.Csv, plug.Defaults); err != nil { return nil, err }; return j.Parse, nil
}
...
// parseWith answers whether src is in the csv language — and,
