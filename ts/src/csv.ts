...
const Csv: Plugin = (tn: Tabnas, options: CsvOptions) => {
  // Human descriptions for CSV tokens, surfaced in railroad diagram legends
  // (read off the live config by @tabnas/railroad). In CSV, newline and
  // whitespace are significant (row separator / field text), and the JSON
  // brace/bracket tokens only appear inside embedded jsonic field values.
  tn.options({
    config: {
      modify: {
        'csv-tokendesc': (cfg: any) => {
