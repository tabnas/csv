...
# Write csv-grammar.jsonic into ts/src/csv.ts, go/csv.go and
...
publish-go: test-go
	...
	sed -i.bak 's/^const VERSION = ".*"/const VERSION = "$(V)"/' go/csv.go
	rm -f go/csv.go.bak
	git add go/csv.go
	git commit -m "go: v$(V)"
	git tag go/v$(V)
...
# not accept a path dependency, so tabnas-csv is not published. Only the
